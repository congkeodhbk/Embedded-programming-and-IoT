...
namespace Framework
{
    public class Util
    {
        public static uint crc32_compute(byte[] bytes)
        {
        ...
        {
            ushort _crc16 = crc16(data, 0, data.Length - 2);
            byte []_crc = BitConverter.GetBytes(_crc16);
            try
            {
                if (_crc[0] == data[data.Length - 2] && _crc[1] == data[data.Length - 1])
                {
                    return true;
                }
            }
    ...
    class customList<T> : List<T>
    {
        ...
        public void appendArray(T[] array, int offset, int len)
        {
